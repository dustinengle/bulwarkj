...
import org.bulwarkj.core.BloomFilter;
import org.bulwarkj.core.ECKey;
import org.bulwarkj.core.NetworkParameters;
import org.bulwarkj.core.Utils;
import org.bulwarkj.crypto.*;
import org.bulwarkj.script.Script;
import org.bulwarkj.utils.Threading;
 ...
 * {@link org.bulwarkj.crypto.DeterministicHierarchy}, to derive all the keys in the keychain from a master seed.
 ...
 * written down on paper or, when represented using a BIP 39 {@link org.bulwarkj.crypto.MnemonicCode},
 ...
 * {@link org.bulwarkj.crypto.DeterministicKey#serializePubB58(org.bulwarkj.core.NetworkParameters)}. The resulting "xpub..." string encodes
 * sufficient information about the account key to create a watching chain via
 * {@link org.bulwarkj.crypto.DeterministicKey#deserializeB58(org.bulwarkj.crypto.DeterministicKey, String, org.bulwarkj.core.NetworkParameters)}
 * (with null as the first parameter) and then
 * {@link DeterministicKeyChain#DeterministicKeyChain(org.bulwarkj.crypto.DeterministicKey,KeyChainType keyChaintype)}.</p>
 *
 * <p>This class builds on {@link org.bulwarkj.crypto.DeterministicHierarchy} and
 * {@link org.bulwarkj.crypto.DeterministicKey} by adding support for serialization to and from protobufs,
...
public class DeterministicKeyChain implements EncryptableKeyChain {
    ...
    public static final ChildNumber BULWARK_PATH = new ChildNumber(119,true);
    public static final ImmutableList<ChildNumber> BIP44_ACCOUNT_ZERO_PATH =
            ImmutableList.of(BIP44_MASTER_KEY, BULWARK_PATH, ChildNumber.ZERO_HARDENED);
    ...
    public DeterministicKeyChain(DeterministicKey watchingKey,KeyChainType keyChainType) {
        ...
        if (keyChainType != KeyChainType.BIP44_BULWARK_ONLY)
    ...
    private void initializeHierarchyUnencrypted(DeterministicKey baseKey) {
        if (baseKey.isPubKeyOnly() && keyChainType == KeyChainType.BIP44_BULWARK_ONLY){
    ...
    public List<DeterministicKey> getKeys(KeyPurpose purpose, int numberOfKeys) {
        ...
        try {
            ...
            for (int i = 0; i < numberOfKeys; i++) {
                ...
                // by any real problem reports from bulwarkj users, but I've heard of cases via the grapevine of
     ...
     * The returned key can be serialized and then passed into {@link #watch(org.bulwarkj.crypto.DeterministicKey)}
    ...
    public static List<DeterministicKeyChain> fromProtobuf(List<Protos.Key> keys, @Nullable KeyCrypter crypter, KeyChainFactory factory) throws UnreadableWalletException {
        ...
        for (Protos.Key key : keys) {
            ...
            if (!path.isEmpty() && path.get(0).equals(BIP44_MASTER_KEY)){
                // is BIP44
                keyChainType = KeyChainType.BIP44_BULWARK_ONLY;
                break;
            }
        }
     ...
     * @see org.bulwarkj.wallet.MarriedKeyChain
     */
